#!/usr/bin/env bash
# Aggregates the test suites of the shipped vendored extensions with their native runners. The root
...
if ! command -v bun >/dev/null 2>&1; then
  echo "ERROR: 'bun' is not on PATH — pi-lsp and pi-ask-user gate on it." >&2
