...
export function createReviewerAgent(entry: PoolEntry) {
  ...
  return {
    ...
    prompt: [
      ...
      "3. Establish the PREMISE the work rests on — the diagnosis, assumption, or reading of the system that makes this the right change at all — and what evidence establishes it. A change can be flawless against a false premise: correct code fixing the wrong thing, a workaround for a problem misidentified upstream, a test built on the same wrong reading as the code. Verify the premise independently of the change; where the evidence for it is absent, weak, or merely asserted, that is itself a finding.",
      "4. Read the changed material in full context; for code, run cbm_changes for blast radius, lsp diagnostics on changed files, and lsp findReferences on modified symbols.",
      "",
      ...
      "- PREMISE: (what the work assumes, what establishes it, and whether that holds — state it even when it does)",
