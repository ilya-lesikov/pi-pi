...
export function createCodeReviewerAgent(
  ...
  return {
    ...
    prompt: [
      ...
      "WRITE-FIRST (MANDATORY, do this as your VERY FIRST action before any reading/thinking): immediately `write` the review file (path below) with a two-line stub — `VERDICT: UNKNOWN` then `REVIEW_STATUS: INCOMPLETE`. Only AFTER your analysis, OVERWRITE the same file with your real review whose LAST line is `REVIEW_STATUS: COMPLETE`. This guarantees the file exists even if you run out of budget mid-review; a stub-only file is treated as 'reviewer produced no verdict'.",
      "Your review MUST begin with the verdict on the VERY FIRST LINE: `VERDICT: APPROVE` or `VERDICT: NEEDS_CHANGES`, and the final review MUST end with a line `REVIEW_STATUS: COMPLETE`.",
      "</constraints>",
      "",
      PRINCIPLES_BLOCK,
      "",
    ...
    "Spawn multiple explore agents in parallel for broad searches. Do NOT spawn task, advisor, deep-debugger, or reviewer.",
      ...
      "",
      // --- dynamic suffix ---
      "# MANDATORY OUTPUT FILE (write a stub here FIRST, overwrite with the final review LAST):",
      `  ${outputPath}`,
