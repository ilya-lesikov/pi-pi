...
function publishFileCommentsBanner(taskDir: string): string {
...
function publishPrCommentsBanner(taskDir: string): string {
  const reviewsDir = join(taskDir, "code-reviews");
  return advanceBanner(
    "[PI-PI] Publish the synthesized review findings as GITHUB PR COMMENTS now, from the user's own `gh`-authenticated account.\n\n" +
    `Use the \`ANCHORS:\` block in the latest \`${reviewsDir}/*_final_pass-*.md\` as the file:line source — do NOT invent locations.\n\n` +
    "For each registered repo, resolve the branch's PR with `gh pr view --json number,headRefOid,url` (skip the repo if there is no PR or `gh auth status` fails). " +
    "Post one line-anchored review comment per accepted finding via `gh api --method POST repos/<owner>/<repo>/pulls/<number>/comments` with `path`, `line`, `side=RIGHT`, and `commit_id=<headRefOid>`. Derive owner/repo from the PR `url` (the base repo). " +
    "A finding whose line GitHub rejects (422 — not part of the PR diff) should be skipped and reported, not retried.\n\n" +
    "The LAST line of every comment body MUST be exactly:\n(generated by pi-pi)\n\n" +
    "IDEMPOTENT: first list existing PR comments (`gh api repos/<owner>/<repo>/pulls/<number>/comments`) and do NOT re-post a comment whose path, line, and finding already match one you previously published (identify yours by the `(generated by pi-pi)` footer). Skip duplicates.\n\n" +
    "When done, report per repo how many comments you posted, skipped as duplicates, and skipped as unmappable, then end your turn.",
  );
}
  ...
  while (true) {
    ...
    if (choice === "Next") {
      ...
      if (finishChoice === "Publish") {
        ...
        const banner = target === "As file comments"
