...
export function planningSystemPrompt(taskDir: string): string {
  ...
  return [
    "[PI-PI — PLAN PHASE]",
    "",
    "Planner outputs are ready.",
    `Read them from ${plansDir}/ and synthesize into a single plan.`,
    "",
    ...
    `1. Read ALL planner outputs from ${plansDir}/`,
    `2. Read ${join(taskDir, "USER_REQUEST.md")} and ${join(taskDir, "RESEARCH.md")} for context`,
