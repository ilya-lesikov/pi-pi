...
export function interactiveFlowBlock(
  ...
  const clarifyStep =
    clarify === "scope"
      ? "1. CLARIFY SCOPE UP-FRONT: if what to review is ambiguous (which repo/branch/PR/commit-range, what to focus on), ask up front before diving in. This is factual scoping, not design elicitation — batch these scoping questions rather than dripping them out. If the scope is clear, skip straight to step 2."
