...
| pi-subagents | `resume` takes an `emitLifecycle` option and is exposed on the published manager handle, so pi-pi can re-point a worker that died on a rate-limited provider and resume it in place — keeping the transcript it had already built — with the resumed run reporting completion the way a spawned one does | `src/agent-manager.ts` (`resume`), `src/index.ts` (manager handle), guarded by `test/agent-manager.test.ts` "reports a resumed run through onComplete" |
