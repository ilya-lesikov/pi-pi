...
| pi-tasks | The task widget keeps to a share of the terminal's rows, for the same reason (`showAll` still prints every task — it is an explicit opt-in) | `src/ui/task-widget.ts` (`rowBudget`), guarded by `test/task-widget.test.ts` "share of a short terminal" |
...
| pi-ask-user | `questions` sequences show a `Question N/M` title and can be walked with alt+←/alt+→ (`AskNavigate` sentinel from the UI layer, index-driven loop in `execute`, revisited answers preselected and overwritten in place) | `index.ts` (`AskNavigate`/`isNavigate`, `NAV_PREV_KEY`/`NAV_NEXT_KEY`, `previousAnswerIndex`, `AskComponent` title/help/`handleInput`, `askUser` `navigation` option, `execute` multi-question loop), guarded by `index.test.ts` "multi-question navigation (local patch)" |
