...
| pi-subagents | Finished agents linger in the widget by wall time (1 h) instead of one main-session turn, the record retention window matches it, and the repaint timer idles to 1 s when nothing is running | `src/ui/agent-widget.ts` (`shouldShowFinished`, `ensureTimer`, `update`), `src/agent-manager.ts` (`cleanup`) |
