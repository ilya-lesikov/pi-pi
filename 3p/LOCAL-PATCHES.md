...
| pi-subagents | Finished agents linger in the widget by wall time (5 min, 10 min for errors) instead of one main-session turn, and the repaint timer idles to 1 s when nothing is running | `src/ui/agent-widget.ts` (`shouldShowFinished`, `ensureTimer`, `update`) |
